...
因為 xterm 無法在緩衝區前端插入,這筆成本必須在 attach 時一次付清。捲到緩衝區
最頂端時,GUI 會告訴你後面還有多少行——本機實測最大的 session 有 93,374 行,
也就是有 68,374 行是緩衝區摸不到的——並提供一個**唯讀的歷史檢視器**,以
4,000 行的視窗(124 欄下約 6 MB,關閉即釋放)翻遍整份 log:⤒/↑/↓/⤓、
Home/End,或是在任一邊界繼續捲動即可。它不需要新的協定訊息:自從 daemon 學會
越過記憶體 ring、直接 seek 磁碟上的 log 之後,同一個 `MSG_SCROLLBACK_REQ`
就能供應任意深度。⌘/Ctrl
